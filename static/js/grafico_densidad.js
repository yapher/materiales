...
// LLAMADA AL BACKEND
// ==========================================================
async function generarGrafico(silencioso = false) {
    const mix = getMezclaActual();
    if (!mix || mix.length === 0) {
        if (!silencioso) {
            mostrarError(
    ...
    if (isNaN(tempMin) || isNaN(tempMax) || isNaN(intervalo)) {
        if (!silencioso) mostrarError("Completá los tres parámetros del rango.");
        return;
    }
    if (tempMax <= tempMin) {
        if (!silencioso) mostrarError("La temperatura máxima debe ser mayor que la mínima.");
        return;
    }
    if (intervalo <= 0) {
        if (!silencioso) mostrarError("El intervalo debe ser mayor que 0.");
        return;
    }
...
function inicializarPanel() {
    const btnGenerar = $("gdBtnGenerar");
    if (btnGenerar) {
        btnGenerar.addEventListener("click", () => generarGrafico(false));
    }
...
window.GraficoDensidad = {
    generar: () => generarGrafico(false),
    generarAutomatico: () => {
        // Se llama cuando el panel se muestra por primera vez
        // tras una predicción.
        renderComposicion();
        generarGrafico(true); // silencioso: no muestra toast
    },
    exportarPDF: exportarPDF,
    renderComposicion: renderComposicion,
};
