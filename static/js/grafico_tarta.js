(function () {
  ...
  /* ==========================================================
     ...
     Gráfico de tarta 3D de la composición de la mezcla.
     Se construye en tiempo real mientras el usuario agrega
     o quita elementos.

     - SVG + JavaScript puro (sin librerías externas).
     - Funciona en desktop (hover) y mobile (tap).
     - Se comunica con mezclas.js a través de
  ...
  const CONFIG = {
    ...
    cy: 116,
    radio: 132,
    inclinacion: 0.52,   // ry = radio * inclinacion (perspectiva 3D)
    profundidad: 34,     // altura de la extrusión 3D
    duracion: 650,       // ms de la animación cuando cambia la mezcla
    elevacionHover: 8,   // px que se eleva una porción al pasar el mouse
    anguloMinimoEtiqueta: 20, // grados mínimos para mostrar etiqueta
  };

  // Accesibilidad: si el usuario pide menos movimiento, sin animación.
  if (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    CONFIG.duracion = 0;
  }

  // Paleta de respaldo (idéntica a COLORES_ELEMENTO de mezclas.js).
  // Si mezclas.js expone la suya (window.MezclasApp.colores), se usa esa.
  ...
  let objetivo = [];               // [{elemento, pct}] mezcla real
  let valoresVisibles = new Map(); // elemento -> pct mostrado (animado)
  let animId = null;
  ...
  function construir() {
    ...
    // Desenfoque para la sombra bajo la tarta.
    const filtro = crear("filter", {
      id: "tartaBlur", x: "-40%", y: "-40%", width: "180%", height: "180%",
    });
    filtro.appendChild(crear("feGaussianBlur", { in: "SourceGraphic", stdDeviation: "7" }));
    capaDefs.appendChild(filtro);

    svg.appendChild(capaDefs);

    // Sombra proyectada debajo de la tarta.
    ...
    capaSlices = crear("g", { class: "tarta-capas" });
    svg.appendChild(capaSlices);

    contenedor.appendChild(svg);

    // Estado vacío (todavía no se agregó ningún elemento).
  ...
  function crearGradiente(id, colorArriba, colorAbajo) {
    const grad = crear("linearGradient", { id, x1: "0", y1: "0", x2: "0", y2: "1" });
    grad.appendChild(crear("stop", { offset: "0%", "stop-color": colorArriba }));
    grad.appendChild(crear("stop", { offset: "100%", "stop-color": colorAbajo }));
  ...
  function asegurarGradientes() {
    ...
    Object.entries(colores).forEach(([elemento, color]) => {
      crearGradiente(`grad-top-${elemento}`, ajustarColor(color, 1.30), ajustarColor(color, 0.94));
      crearGradiente(`grad-side-${elemento}`, ajustarColor(color, 0.66), ajustarColor(color, 0.42));
    });
    // Porción "restante" (lo que falta para llegar a 100%).
  ...
  function caminoTapa(a0, a1) {
    ...
    // Caso "círculo completo" (un solo elemento al 100%).
    if (barrido >= 359.9) {
      const p0 = punto(a0);
  ...
  function caminoPared(a0, a1) {
    // La pared lateral solo es visible en la mitad frontal (0° a 180°).
  ...
  function dibujarSlice(item, a0, a1) {
    ...
    // Pared lateral (solo si tiene parte visible al frente).
    const pared = caminoPared(a0, a1);
    if (pared) {
      grupo.appendChild(crear("path", {
        class: "tarta-pared",
        d: pared,
        fill: item.restante ? "url(#grad-rest-side)" : `url(#grad-side-${item.id})`,
      }));
    }
    ...
    grupo.appendChild(crear("path", {
      class: "tarta-tapa",
      d: caminoTapa(a0, a1),
      fill: item.restante ? "url(#grad-rest-top)" : `url(#grad-top-${item.id})`,
    }));

    // Etiqueta sobre la porción (solo si es lo bastante grande).
    if (!item.restante && barrido >= CONFIG.anguloMinimoEtiqueta) {
  ...
  function render() {
    ...
    // Las porciones se regeneran en cada frame: limpiar referencia activa.
    sliceActiva = null;
    if (tooltip) tooltip.classList.remove("tooltip-visible");

    while (capaSlices.firstChild) capaSlices.removeChild(capaSlices.firstChild);

    const total = [...valoresVisibles.values()].reduce((a, b) => a + b, 0);
    const hayDatos = total > 0.05;
    ...
    if (badgeEstado) {
      badgeEstado.textContent = `${formatearNumero(total)}%`;
      badgeEstado.classList.toggle("estado-completo", completo);
      badgeEstado.classList.toggle("estado-excedido", excedido);
    }

    if (!hayDatos) {
      svg.setAttribute("aria-label", "Composición vacía: agregá elementos a la mezcla");
      return;
    }

    // Referencia SIEMPRE sobre 100%: así la tarta "se va armando".
    const referencia = Math.max(total, 100);

    const items = [];
    objetivo.forEach((o) => {
      const pct = valoresVisibles.get(o.elemento) || 0;
      if (pct > 0.01) items.push({ id: o.elemento, pct, restante: false });
    });
    // Elementos que están desapareciendo (animación de salida).
    valoresVisibles.forEach((pct, elemento) => {
      const enObjetivo = objetivo.some((o) => o.elemento === elemento);
      if (!enObjetivo && pct > 0.01) items.push({ id: elemento, pct, restante: false });
    });

    // Porción fantasma con lo que falta para el 100%.
    const restante = Math.max(0, 100 - total);
    if (restante > 0.05 && !excedido) {
      items.push({ id: ID_RESTANTE, pct: restante, restante: true });
    }

    let angulo = -90; // arranca desde arriba, sentido horario
    items.forEach((item) => {
  ...
  function animarHacia(nuevaMezcla) {
    ...
    if (CONFIG.duracion <= 0) {
      valoresVisibles = new Map(
        objetivo.filter((o) => o.pct > 0).map((o) => [o.elemento, o.pct])
      );
      render();
      return;
    }
    ...
    const claves = new Set([...desde.keys(), ...objetivo.map((o) => o.elemento)]);
    ...
    function paso(ahora) {
      ...
      claves.forEach((clave) => {
        ...
        if (valor > 0.01) nuevos.set(clave, valor);
      });
      ...
      if (t < 1) {
        ...
        animId = null;
        // Estado final limpio (sin elementos que desaparecieron).
        const finales = new Map();
        objetivo.forEach((o) => {
          if (o.pct > 0.01) finales.set(o.elemento, o.pct);
        });
  ...
  /* ==========================================================
     INTERACCIÓN (hover en desktop / tap en mobile)
     ========================================================== */

  function conectarEventos() {
    svg.addEventListener("pointerover", (ev) => {
      const grupo = ev.target.closest ? ev.target.closest(".tarta-slice") : null;
      if (!grupo || grupo.classList.contains("tarta-restante")) {
        desactivarSlice();
        return;
      }
    ...
    svg.addEventListener("pointermove", (ev) => {
      if (sliceActiva) posicionarTooltip(ev.clientX, ev.clientY);
    });

    svg.addEventListener("pointerleave", desactivarSlice);

    // En touch: ocultar el tooltip si se toca fuera del gráfico.
    document.addEventListener("pointerdown", (ev) => {
      if (contenedor && !contenedor.contains(ev.target)) desactivarSlice();
    }, true);
  }
  ...
  function activarSlice(grupo, clientX, clientY) {
    ...
    sliceActiva = grupo;
    capaSlices.appendChild(grupo); // traer al frente
    grupo.classList.add("tarta-slice-activa");

    // Elevar la porción hacia afuera, siguiendo su ángulo medio.
  ...
  function mostrarTooltip(elemento, clientX, clientY) {
    if (!tooltip) return;
    const colores = conseguirColores();
    const color = colores[elemento] || "#88c999";
    const pct = valoresVisibles.get(elemento) || 0;
    tooltip.innerHTML = `
      <span class="grafico-tarta-tooltip-chip" style="background:${color};"></span>
      <strong>${elemento}</strong>
      <span class="grafico-tarta-tooltip-pct">${formatearNumero(pct)}%</span>
    `;
  ...
  function posicionarTooltip(clientX, clientY) {
    ...
    // Mantener el tooltip dentro del contenedor.
  ...
  /* ==========================================================
     API PÚBLICA (la llama mezclas.js)
     ========================================================== */
  ...
  function init() {
    ...
    // Si ya había una mezcla restaurada, dibujarla directo.
    if (window.MezclasApp && typeof window.MezclasApp.getMix === "function") {
      ...
      if (Array.isArray(mix) && mix.length > 0) {
        ...
        valoresVisibles = new Map(objetivo.map((o) => [o.elemento, o.pct]));
        render();
      }
    }
  }
